...
class AeotecWallmoteDuoDevice extends ZwaveDevice {
	
	onMeshInit() {
        this._batteryTrigger = new Homey.FlowCardTriggerDevice('zw129_battery_full')
            .register();
        ...
        this.registerReportListener('CONFIGURATION', 'CONFIGURATION_REPORT', (report) => {
            if (report.hasOwnProperty('Parameter Number') &&
                ...
                if (report['Parameter Number'] === 10) {
                    ...
                    if (value < .5) value = Math.max(value - .05, 0);
                    const token = {
                        dim: value
                    };
                    const data = {
    ...
    async onSettings(oldSettings, newSettings, changedKeys) {
        if (newSettings.rgb_name === 'custom') {
            ...
            let valueArray = newSettings.rgb_name.split(',');
            let multiplier = newSettings.rgb_name_level / 100 || 1;
...
module.exports = AeotecWallmoteDuoDevice;